...
设前缀和数组为 ![\textit{preSum} ](./p__textit{preSum}_.png) ，则问题等价于求所有的下标对 *(i,j)*，满足
![\textit{preSum}\[j\]-\textit{preSum}\[i\]\in\[\textit{lower},\textit{upper}\] ](./p___textit{preSum}_j__-_textit{preSum}_i__in__textit{lower},_textit{upper}___.png) 

我们先考虑如下的问题：给定两个**升序排列**的数组 *n_1, n_2*，试找出所有的下标对 *(i,j)*，满足
![n_2\[j\]-n_1\[i\]\in\[\textit{lower},\textit{upper}\] ](./p___n_2_j__-_n_1_i__in__textit{lower},_textit{upper}___.png) 

在已知两个数组均为升序的情况下，这一问题是相对简单的：我们在 *n_2* 中维护两个指针 *l,r*。起初，它们都指向 *n_2* 的起始位置。

随后，我们考察 *n_1* 的第一个元素。首先，不断地将指针 *l* 向右移动，直到 ![n_2\[l\]\gen_1\[0\]+\textit{lower} ](./p__n_2_l__ge_n_1_0__+_textit{lower}_.png)  为止，此时， *l* 及其右边的元素均大于或等于 ![n_1\[0\]+\textit{lower} ](./p__n_1_0__+_textit{lower}_.png) ；随后，再不断地将指针 *r* 向右移动，直到 ![n_2\[r\]>n_1\[0\]+\textit{upper} ](./p__n_2_r____n_1_0__+_textit{upper}_.png)  为止，则 *r* 左边的元素均小于或等于 ![n_1\[0\]+\textit{upper} ](./p__n_1_0__+_textit{upper}_.png) 。故区间 *[l,r)* 中的所有下标 *j*，都满足 
![n_2\[j\]-n_1\[0\]\in\[\textit{lower},\textit{upper}\] ](./p___n_2_j__-_n_1_0__in__textit{lower},_textit{upper}___.png) 
...
- 时间复杂度：![O(N\logN) ](./p__O_Nlog_N__.png) ，其中 *N* 为数组的长度。设执行时间为 *T(N)*，则两次递归调用的时间均为 *T(N/2)*，最后需要 *O(N)* 的时间求出下标对数量以及合并数组，故有
    ![T(N)=2\cdotT(N/2)+O(N) ](./p_______T_N__=_2_cdot_T_N_2__+_O_N_______.png) 
    根据主定理，有 ![T(N)=O(N\logN) ](./p__T_N__=_O_Nlog_N__.png) 。

- 空间复杂度：*O(N)*。设空间占用为 *M(N)*，递归调用所需空间为 *M(N/2)*，而合并数组所需空间为 *O(N)*，故
    ![M(N)=\max\big\{M(N/2),O(N)\big\}=M(N/2)+O(N) ](./p_______M_N__=_maxbig{M_N_2_,_O_N_big}_=_M_N_2__+_O_N_______.png) 
    根据主定理，有 *M(N) = O(N)*。
...
依然考虑前缀和数组 ![\textit{preSum} ](./p__textit{preSum}_.png) 。

对于每个下标 *j*，以 *j* 为右端点的下标对的数量，就等于数组 ![\textit{preSum}\[0..j-1\] ](./p__textit{preSum}_0..j-1__.png)  中的所有整数，出现在区间 ![\[\textit{preSum}\[j\]-\textit{upper},\textit{preSum}\[j\]-\textit{lower}\] ](./p___textit{preSum}_j_-textit{upper},_textit{preSum}_j_-textit{lower}__.png)  的次数。故很容易想到基于线段树的解法。

我们从左到右扫描前缀和数组。每遇到一个数 ![\textit{preSum}\[j\] ](./p__textit{preSum}_j__.png) ，我们就在线段树中查询区间 ![\[\textit{preSum}\[j\]-\textit{upper},\textit{preSum}\[j\]-\textit{lower}\] ](./p___textit{preSum}_j_-textit{upper},_textit{preSum}_j_-textit{lower}__.png)  内的整数数量，随后，将 ![\textit{preSum}\[j\] ](./p__textit{preSum}_j__.png)  插入到线段树当中。
...
- 时间复杂度：![O(N\logN) ](./p__O_Nlog_N__.png) 。使用哈希离散化之后，线段树维护的区间大小为 *O(N)*，故其深度、单次查询或插入的时间复杂度均为 ![O(\logN) ](./p__O_log_N__.png) 。而离散化本身的复杂度也为 ![O(N\logN) ](./p__O_Nlog_N__.png) 。

- 空间复杂度：*O(N)*。线段树的深度为 *O(N)*，而第 *i* 层拥有的节点数量为 *2^{i-1}*，故线段树总的节点数量为 ![2^{O(\logN)}=O(N) ](./p__2^{O_log_N_}_=_O_N__.png) 。
...
- 时间复杂度：![O(N\logC) ](./p__O_N_log_C__.png) ，其中 *C* 是线段树根节点对应的区间长度。由于我们使用 *64* 位整数类型进行存储，因此 ![\logC ](./p__log_C_.png)  不会超过 *64*。使用动态增加节点的线段树，单次查询或插入的时间复杂度均为 ![O(\logC) ](./p__O_log_C__.png) 。
- 空间复杂度：![O(N\logC) ](./p__O_N_log_C__.png) 。需要进行 *N* 次线段树的插入操作，每次会添加不超过 ![\logC ](./p__log_C_.png)  个新节点。

#### 方法四：树状数组

**思路与算法**

树状数组与线段树基于类似的思想，不过树状数组支持的基本查询为求出 ![\[0,\textit{val}\] ](./p___0,_textit{val}__.png)  之间的整数数量。为了查询区间 ![\[\textit{preSum}\[j\]-\textit{upper},\textit{preSum}\[j\]-\textit{lower}\] ](./p___textit{preSum}_j_-textit{upper},_textit{preSum}_j_-textit{lower}__.png)  内的整数数量，需要执行两次查询，即分别查询 ![\[0,\textit{preSum}\[j\]-\textit{upper}-1\] ](./p___0,_textit{preSum}_j_-textit{upper}-1__.png)  区间的整数数量 *L* 和![\[0,\textit{preSum}\[j\]-\textit{lower}\] ](./p___0,textit{preSum}_j_-textit{lower}__.png)  区间的整数数量 *R*，答案即为两者作差 *R-L*。
...
- 时间复杂度：![O(N\logN) ](./p__O_Nlog_N__.png) 。离散化本身的复杂度为 ![O(N\logN) ](./p__O_Nlog_N__.png) ，而树状数组单次更新或查询的复杂度为 ![O(\logN) ](./p__O_log_N__.png) 。
...
考虑一棵平衡二叉搜索树。若其节点数量为 *N*，则深度为 ![O(\logN) ](./p__O_log_N__.png) 。二叉搜索树能够在 ![O(\logN) ](./p__O_log_N__.png)  的时间内，对任意给定的值 ![\textit{val} ](./p__textit{val}_.png) ，查询树中所有小于或等于该值的数量。

因此，我们可以从左到右扫描前缀和数组。对于 ![\textit{preSum}\[j\] ](./p__textit{preSum}_j__.png)  而言，首先进行两次查询，得到区间 ![\[\textit{preSum}\[j\]-\textit{upper},\textit{preSum}\[j\]-\textit{lower}\] ](./p___textit{preSum}_j_-textit{upper},_textit{preSum}_j_-textit{lower}__.png)  内的整数数量；随后再将 ![\textit{preSum}\[j\] ](./p__textit{preSum}_j__.png)  插入到平衡树中。
